...
parser.add_argument("-f","--file", type=lambda p: Path(p).absolute(), help=r"Path (relative or absolute) to the metadata_sqlite_db file. Example: C:\Data\metadata_sqlite_db")
parser.add_argument("-d","--directory", type=lambda p: Path(p).absolute(), help=r"Path (relative or absolute) to directory containing the cache. Example: C:\Data\content_cache")
parser.add_argument("-o","--output", type=lambda p: Path(p).absolute(), help=r"Path (relative or absolute) to directory were the output shoudl be written to. Example: C:\Data")
